...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    // 2. LẮNG NGHE SỰ KIỆN AUTH - FIX CHÍ MẠNG CHO MICROSOFT OAUTH
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (session) {
        setUser(session.user);
        
        // Khi Supabase bóc tách thành công Token từ Microsoft gửi về (Sự kiện SIGNED_IN)
        if (event === "SIGNED_IN") {
          const expiresAt = Math.floor(Date.now() / 1000) + 21600;
          localStorage.setItem("session_expires_at", expiresAt.toString());
          
          // Ép ứng dụng chuyển hướng thẳng vào /map ngay lập tức để tránh bị LoginPage chặn lại
    ...
    // 3. Vòng lặp kiểm tra định kỳ mỗi phút một lần
