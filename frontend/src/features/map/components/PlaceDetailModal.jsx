...
export default function PlaceDetailModal({ place, onClose, onStatusUpdated, apiKey, openedFromBuilding = null, onBackToBuilding = null }) {
  ...
  // từ MapPage sau khi admin approve request). Không ghi đè khi đang ở Edit mode
  // để tránh mất dữ liệu người dùng đang gõ dở.
  useEffect(() => {
  ...
  const handleDeletePlace = async () => {
    if (!isOwner) {
      showToast("You don't have permission to delete this place", "error");
      return;
    }
              ...
              place.opening_hours.length > 0 && (
                <>
                  ...
                  <div className="py-4">
                    ...
                    {/* Current status */}
                    <div
                      className={`inline-flex w-fit items-center gap-2 rounded-full px-3 py-1.5 text-sm font-semibold ${getOpeningStatusColor(
                        place.opening_hours
                      )}`}
                    >
                      <span
